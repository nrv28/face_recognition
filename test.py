...
# # Sample names corresponding to detected faces
# person_names = ["Nirjay", "Nishant", "Person 3"]
...
#         # Compare the face region with known images
#         match_results = [cv2.matchTemplate(face_roi, cv2.cvtColor(known_image, cv2.COLOR_BGR2GRAY), cv2.TM_CCOEFF_NORMED) for known_image in known_images]
...
#             # Check if the best match score exceeds the threshold and if the index is valid
#             # if best_match_index < len(person_names) and match_results[best_match_index] > 0.7:
#                 # Display the name if a match is found
#             name = person_names[best_match_index]
#                 # print(name)
